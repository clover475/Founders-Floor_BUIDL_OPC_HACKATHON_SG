      ...
      <section className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        {rooms.map((room) => (
          <Link
            key={room}
            href={`/office/${room}`}
            className="group border border-floor-line bg-white/70 p-4 transition hover:border-floor-green hover:bg-white"
          >
            <div className="mb-5 flex h-9 w-9 items-center justify-center bg-floor-panel text-floor-green">
              <UsersRound size={19} aria-hidden="true" />
            </div>
            <h2 className="inline-flex items-center gap-2 text-base font-semibold text-floor-ink">
              {roomT(`${room}.name`)}
